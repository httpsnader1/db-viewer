<template>
  <DbViewerLayout :breadcrumbs="[$t('db_viewer_title'), $t('dashboard')]">
    <div class="space-y-8">
      <!-- Page Title & Date Filter -->
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-white tracking-tight">{{ $t('database_overview') }}</h1>
          <p class="text-sm text-gray-400 mt-1">
            {{ $t('connected_to') }} <span class="text-violet-400 font-medium">{{ stats.database }}</span>
            {{ $t('via') }} <span class="text-gray-300 font-medium">{{ stats.driver }}</span>
          </p>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              ...
              <div class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('total_records') }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <div class="flex items-center justify-between mb-0">
            ...
            <div class="text-right">
              ...
              <div class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('total_tables') }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              ...
              <div class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('db_driver') }}</div>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <div>
              ...
              <p class="text-2xl font-black text-white mt-1">{{ chart.total.toLocaleString() }} <span class="text-xs font-normal text-gray-500">{{ $t('records') }}</span></p>
            </div>
          </div>
      ...
      <div class="rounded-2xl bg-gray-900 border border-gray-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <h2 class="text-sm font-bold text-gray-300">{{ $t('popular_tables') }}</h2>
          <Link
...
const dateRangeText = computed(() => {
  if (!dateRange.value || !dateRange.value[0] || !dateRange.value[1]) return usePage().props.db_viewer.translations['select_date_range'] || 'Select Date Range';
  const locale = usePage().props.db_viewer.locale || 'en-US';
  const options = { year: 'numeric', month: 'short', day: 'numeric' };
  return `${dateRange.value[0].toLocaleDateString(locale, options)} - ${dateRange.value[1].toLocaleDateString(locale, options)}`;
})
